		...
		if (turn === 'player') {
			...
			while (!playerOption) {
				...
				switch (playerOption) {
					case '1':
						console.log(
							`You chose to ${color('attack', 'white')}, and dealt ${color(player.str.toString(), 'yellow')} ATK to ${color(monsters[currentFloor][0].name, 'red')}.`,
						);
					...
					case '2':
						...
						console.log(`You chose to ${color('heal', 'white')}, you regain ${color(remaining_hp.toString(), 'green')} HP.`);
						break;
				}
			}
            turn = 'monster';
            ...
            console.clear();
            console.log(`${color(monsters[currentFloor][0].name, 'red')} is thinking about his attack.`);
            press_to_continue();
            player.hp -= monsters[currentFloor][0].str;
            console.clear();
            console.log(`${color(monsters[currentFloor][0].name, 'red')} attacked you and you lost ${color(monsters[currentFloor][0].str.toString(), 'yellow')} HP.`)
            turn = 'player';
            press_to_continue();
        }
	}
